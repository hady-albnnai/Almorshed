...
النوطة صارت **ملف PDF نهائياً من 79 صفحة A4** (HTML دلالي RTL + CSS طباعة + Chromium) مبنيّاً من المصدر المجمّد بلا أي مساس به، ومعها منظومة تحقّق مستقلة **نجحت في كل فحوص التحقّق النهائي (7/7)**.

- **NHTML-0** (جرد) ✅ ثم **إصلاحات جرد** ✅ (نص المربعات، تتبّع أبناء المجموعات، الفحوصات الفعالة، **منع تصادم معرّفات الكتل**).
- **NHTML-5** (التحقّق النهائي) ✅ — 0 حرف مفقود · 75/75 صورة مطابقة بصرياً · 22/22 رمزاً · تذييل 79/79 · مسح بصري لكل الصفحات · 3 عيوب شكل مُصلَحة.
...
**اكتملت المراحل NHTML-0…NHTML-5.** المشروع جاهز للتسليم: `pdf/نوطة-النواسات.pdf` (79 صفحة) + `html/index.html` + `html/preview-lite.html` للمعاينة + `reports/NHTML-5-final-validation.md`.

**توصية متابعة (اختيارية):** إن رغب المالك بتقليل عدد الصفحات أو تعديل تصميم التسميات/الصناديق (ألوان السمة), فهذا تغيير شكل جديد يفتح مرحلة سادسة بقرار منه — لا يُنفَّذ تلقائياً.
...
## الملفات الناتجة (NHTML-5)

| الملف | المضمون |
|---|---|
| `rebuild/nawwasat/tools/final_validation.py` | مدقّق NHTML-5 (نصّ · صور بصريّة · رموز HTML↔PDF · ألوان · ترقيم · مسح بصري + ألواح) |
| `rebuild/nawwasat/content/final-validation.json` | نتيجة التحقّق النهائي — **verdict 7/7 PASS** |
| `rebuild/nawwasat/reports/NHTML-5-final-validation.md` | تقرير المرحلة (المنهج · الجداول · العيوب المُصلَحة · القيود) |
| `rebuild/nawwasat/reports/nhtml5-shots/` | 7 ألواح تغطّي كل الصفحات + لقطات تكبير |
