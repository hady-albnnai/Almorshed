...
**NHTML-4 — إخراج PDF** عبر Playwright/Chromium: `pdf/نوطة-النواسات.pdf` (A4 · `printBackground` · انتظار الخطوط والصور + `window.__fitMath()` قبل الإخراج · قالب تذييل بسطر الأستاذ حرفياً + رقم الصفحة)، ثم NHTML-5 (تحقّق نهائي + مسح بصري لكل صفحة).
